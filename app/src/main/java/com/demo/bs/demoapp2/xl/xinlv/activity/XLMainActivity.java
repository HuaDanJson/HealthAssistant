package com.demo.bs.demoapp2.xl.xinlv.activity;
...
import com.demo.bs.demoapp2.xl.monitor.utils.ImageProcessing;
...
public class XLMainActivity extends Activity {
	...
	private static String s_beatsAvg="";//平均心率
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_xlmain);
		initConfig();
		findViewById(R.id.btn).setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				setResult(1,new Intent().putExtra("data",s_beatsAvg));
				finish();
			}
		});
	}
...
//			addY=250;
			...
			if(gx<200){
				if(hua[20]>1){
					Toast.makeText(XLMainActivity.this, "请用您的指尖盖住摄像头镜头！", Toast.LENGTH_SHORT).show();
					hua[20]=0;}
				hua[20]++;
				return;}
			else
	...
	private static PreviewCallback previewCallback = new PreviewCallback() {
		public void onPreviewFrame(byte[] data, Camera cam) {
			...
			if (totalTimeInSecs >= 2) {
				...
				//获取系统时间（ms）
				s_beatsAvg=String.valueOf(beatsAvg);
				startTime = System.currentTimeMillis();
				beats = 0;
			}
			processing.set(false);
